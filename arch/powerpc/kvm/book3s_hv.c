				 ...
				 struct task_struct *tsk)
{
	 ...
	 */
	if (!kvmhv_is_nestedv2() && (kvmppc_get_msr(vcpu) & MSR_HV)) {
	...
	case BOOK3S_INTERRUPT_SYSCALL:
	{
		...
		if (!kvmhv_is_nestedv2() && unlikely(kvmppc_get_msr(vcpu) & MSR_PR)) {
			/*
			  ...
			  unsigned long lpcr)
{
	...
	if (!nested) {
		...
		if (test_bit(BOOK3S_IRQPRIO_EXTERNAL,
			 ...
			 */
			if (!kvmhv_on_pseries() && (kvmppc_get_msr(vcpu) & MSR_EE))
...
static int kvmppc_vcpu_run_hv(struct kvm_vcpu *vcpu)
{
	...
	do {
		...
		if (run->exit_reason == KVM_EXIT_PAPR_HCALL) {
			...
			if (!kvmhv_is_nestedv2() && WARN_ON_ONCE(kvmppc_get_msr(vcpu) & MSR_PR)) {
				/*
